...
**Note on this copy (Route & Stamps):** only `styles.css`, `theme.json`, and this guide were handed off — the example HTML pages (`foundations/`, `components/`, `templates/`), `thumbnail.html`, and `print-plates.js` were not provided. Per ARCHITECTURE.md §1b, `print-plates.js`/`.cmyk` is optional at MVP scale anyway; use `.halftone` for interface imagery instead. The component table above is the reference for class names and structure until real example markup is available.
