...
**Push notifications — scoped (2026-08-21), not built.** Real OS-tray notifications while the app isn't open — materially bigger than it sounds, since M7's existing notification system (`notifications` table, instant/digest paths, `NotificationBell`) is entirely in-app and has no path to a closed app at all. Promoted out of the one-line "Beyond M9" mention below into a real scope, same treatment Milestone G's chat assistant got.

- `push_subscriptions` table (`user_id`, `endpoint`, `p256dh`, `auth`, `created_at`) — one row per device/browser a user has granted permission on, since a person can have more than one.
- A VAPID keypair (`web-push` package's `generateVAPIDKeys()`) — public key safe to ship to the client, private key as a Vercel-only server secret. Free, keyless from the browser's perspective, no paid push service involved — stays inside the $0/month ground rule.
- `sw.ts` (Serwist) gains a `push` event handler calling `self.registration.showNotification(...)` — this is the actual mechanism that reaches the OS tray with the app closed — and a `notificationclick` handler to focus/open the app on tap.
- The send trigger has to stay instant for the consensus-vote case (M7's own acceptance criterion), which rules out a polling cron like `check_scheduled_arrivals()` uses — a delay there would quietly break that guarantee. Instead: a Postgres trigger on `notifications` INSERT calling out via `pg_net` (available on Supabase) to a Supabase Edge Function that actually sends the push via `web-push`, keeping the same "fires the moment the row is inserted" behavior the in-app system already has.
- Permission UX: browsers require an explicit user gesture — a "Enable notifications" toggle on the Profile page (already the natural home for account-level preferences per Milestone A) triggering `Notification.requestPermission()`, not an unprompted request on load.
- **Platform note, genuinely different from Web Share Target/the install banner**: iOS 16.4+ Safari *does* support standard Web Push for a PWA already added to the home screen (no paid Apple Developer account needed, unlike native APNs) — but only once installed, not from a regular Safari tab. Not automatically excluded from iOS the way Share Target is; worth building for both once iOS gets its own pass.

Not scoped yet: exactly which existing notification types (consensus, packing/arrival reminders, join notifications) should also fire a push vs. stay in-app-only, and whether the digest path should push at all or stay email-only. Flag when this gets picked up for real.
...
Multi-trip accounts, request-based invites, standalone tripless places, voting-visibility model for friend-group scale. These live in PRD §7/§14 as intentionally deferred — pick this roadmap back up with a new M10 when one of them is actually next. (Push notifications moved to the "Deferred — not scoped yet" section above — it now has a real scope, just not built.)
