...
- Trip description (text, nullable) and date range (`start_date`/`end_date`, nullable) added to `trips` — additive columns, editable from the same Settings route, reusing the native date-input pattern from Milestone D. Surfaced read-only on the Route page header, since that's the first thing opened each session (revisit placement if that turns out to be the wrong spot)
