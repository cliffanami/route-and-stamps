...
### Q — Packing page, redefined (categories, collapsible sections, item description) — shipped (2026-08-26)
...
1. **Category picker.** A `packing_categories` array on `trips`, same shape as the already-shipped `currencies`/`tip_categories`/`budget_categories` — checked against the live database while building this, and that trio's own Milestone A fast-follow had *never actually included packing* (`packing_categories` didn't exist as a column at all until this migration). Editable via a fourth `TagListEditor` in Trip Settings' "Currencies & categories" section. `PackingForm`'s category field is now a `<select>` sourced from that list, same strict-select pattern and "keep a legacy value selectable if it's not in the current list" handling every other categorized field already uses. New trips default to a starter list (`Pre-trip`/`Packing list`/`On-trip`) — but the one real trip's actual categories turned out to be different from that starter set (`App`/`Packing list`/`Visa & Documents`, the largest bucket by far at 21 items), so its row was seeded to match what's *actually* in use rather than forcing a rename onto real data.
2. **Collapsible sections**, built against that now-ordered category list — `PackingMatrix` used to group items by first-seen order; it now follows `packing_categories`' configured order, with any legacy/off-list category (an old free-text value, or the "General" fallback for an uncategorized item) still getting its own section, appended after the configured ones rather than dropped. Each heading gets a collapse toggle (chevron), expanded by default, state kept client-side for the session only.
3. **Item description.** New `packing_items.description` column, optional, shown as truncated muted text under the item name in the matrix (full text in the existing item-detail dialog) and editable via a new field in `PackingForm`.

**Acceptance:** adding a packing item offers a category dropdown instead of free text (verified live); a new trip's packing categories start pre-populated with the three starter sections (verified: confirmed against the DB column default, not just app behavior); existing free-text category values on already-seeded items keep working, shown as their own section rather than silently dropped or merged (verified live: a legacy category renders after the configured ones); clicking a category heading collapses/expands just that section, nothing collapsed by default (verified live, both directions); a packing item can carry an optional description, visible under its name in the matrix (verified live).

**Follow-up (2026-08-26):** live-usage feedback surfaced immediately after shipping — a strict-select category picker with no way to add a missing category *without leaving the form* is a dead end the moment someone needs a category that doesn't exist yet. `PackingForm`'s category `<select>` gained a "+ Add a new category…" option that reveals an inline name field; submitting it calls the same full-row `useUpdateTripCategoryConfig` mutation Trip Settings' own `TagListEditor`s use (appending to `packing_categories`, not replacing it), then selects the new category immediately — no dialog-closing round trip through Settings. Verified live: the new category is both selected in-form and actually persisted to `trips.packing_categories` alongside the starter list, and a name matching an existing category selects it instead of creating a duplicate (same silent-dedup behavior `TagListEditor` already uses). This same gap exists identically in `TipForm`, `BudgetForm`, `StopLogisticsForm`'s transport-mode select, and the trip currency picker — flagged, not yet fixed there; scoped to packing only per the explicit ask.
...
### V — Splash screen icon fix (Pixel) — shipped, pending real-device confirmation (2026-08-26)

**Goal:** live-usage feedback: "on the pixel the logo that comes up on the splash screen is off." A real rendering bug, not a scoping question.

- **Found**: the manifest's `purpose: "maskable"` icon entries reused the exact same files as the `purpose: "any"` entries — no dedicated safe-zone margin at all. Android's adaptive-icon mask (used for both the home-screen icon and the Android 12+ splash screen) can crop anything outside a centered circle at 80% of the icon's diameter; reusing an icon sized for unmasked display as "maskable" too risks the mark getting clipped by a launcher's mask shape.
- **Fix**: two new dedicated maskable files (`icon-192-maskable.png`, `icon-512-maskable.png`) with the visible mark sized at 56% diameter (28% radius) — comfortably inside the 80% safe zone, more margin than the strict minimum. The `any`-purpose icons are untouched in role (still the plain, unmasked mark, at a slightly larger 72% diameter for visual balance now that it's not sharing files with the maskable ones). Manifest updated to point `purpose: "maskable"` at the new files instead of reusing the `any` ones. Both placeholder icons (per the file's own existing comment — not real branding, generated with no design input), regenerated by rendering SVG through a headless browser rather than needing an image-editing tool.
- **Honest limitation**: this environment has no real Android device or Pixel emulator — the fix targets the most common, well-documented cause of exactly this symptom (a maskable icon with no safe-zone margin), verified by confirming the manifest now serves genuinely distinct maskable files with the expected geometry, but it is *not* confirmed against an actual Pixel splash screen. Per this milestone's own acceptance bar, that confirmation still needs to happen on your end.

**Acceptance:** the manifest declares distinct, safe-zone-correct maskable icons (done, verified via the live manifest response and the rendered PNGs); the PWA splash screen renders the logo correctly centered/uncropped on a real Pixel — please confirm after reinstalling/updating the PWA, and let me know if it's still off (ideally with a screenshot — devtools-only checking can't reproduce this class of bug).

---

### W — Place dates and a day-by-day itinerary view — shipped (2026-08-26)
...
- New `places.date date` column, optional — independent of the stop's own `start_date`/`end_date` range, not enforced against it. Settable directly in Place Detail's edit form (`EditPlaceDetailsForm`), or via the must-go date prompt below.
- **New `ItineraryView` component**, separate from Stop Detail's existing free-text "Day by day" narrative (untouched — still Milestone J's `splitDayNarrative`, still reads a stop's description). Groups dated places by their `date` under a heading per day, undated places listed separately under "Not yet scheduled." Rendered on the Route page, below the existing per-stop/Unassigned sections — hidden entirely until at least one place has a date, so a fresh trip doesn't get a redundant "everything is unscheduled" list duplicating what's already shown above it.
- **Must-go consensus triggers a date prompt** — the first auto-triggered `Dialog` in the app (every other dialog opens from a button click). Reuses the existing `isMutualMustGo` check (`PlaceRow.tsx`): `PlaceRow` now computes the before/after mutual-must-go state synchronously around each vote cast (mirroring `CheckInControl`'s `onCheckedIn` pattern — captured from already-rendered vote data, not a refetch) and fires `onMustGoConsensus` only on the not-mutual → mutual transition, never on every vote. `RouteSpine` opens `MustGoDatePrompt` ("When do you want to go?") in response. Skipping just closes the dialog with no mutation — the date stays optional either way.
...
**Goal:** once places carry dates (Milestone W), "what's planned for tomorrow" becomes answerable. Scope narrowed during build: ship the Route page banner now; hold off on a new "tomorrow's plan" push notification type; but since fixing the two pre-existing *dead* scheduled notifications (`arrival_estimated`, `packing_due` — see Milestone D) needs the exact same cron infrastructure, build that now and use it for the fix, not the new feature.

- **Route page banner**: `TomorrowBanner` — a "Tomorrow" card at the top of the Route page listing places dated for the next calendar day, visible to both trip members whenever they open the app. No new infrastructure, just a query against `places.date` already loaded by `RouteSpine`.
- **The first working cron in the app.** Checked against the live database while scoping this: `pg_cron` was not installed, and `check_scheduled_arrivals()`/`check_packing_reminders()` (Milestone D) had never actually been invoked by anything — those two notification types have never fired in production. Fixed with a genuinely new mechanism: a Vercel Cron Job (`vercel.json`, daily at 00:00 UTC = 09:00 JST, a reasonable morning-check time for the Japan trip) hitting a new Route Handler (`/api/cron/daily-checks`) that calls both functions via RPC. This is a legitimate new category of Route Handler, not a CONVENTIONS.md §1 violation — a scheduled server-to-server trigger has no user session and no client to make "just a Supabase call" from.
- **Two new secrets, flagged rather than silently added.** The Route Handler runs with no user session, so it needs the Supabase **service-role key** to call these functions reliably — this app had never used that key from Next.js before (only the isolated `send-push` Edge Function did), so this was confirmed explicitly before adding `SUPABASE_SERVICE_ROLE_KEY` as a new server-only env var, scoped to just this one file. A `CRON_SECRET` env var gates the endpoint (Vercel automatically sends it as a bearer token on cron-triggered requests once the var exists), so an arbitrary request can't trigger it.
- **No new push notification type shipped.** "Tomorrow's plan" push was explicitly deferred — the banner is the only surfacing of that specific ask for now. The cron's only job today is firing the two already-existing notification types correctly.

**Acceptance:** the Route page shows a "Tomorrow" card listing the next day's dated places (verified live: shows a place dated tomorrow, correctly excludes one dated today); the cron endpoint rejects requests without the correct secret (verified: 401 without/with a wrong `Authorization` header, 200 with the correct one) and, when called, actually creates `arrival_estimated`/`packing_due` notification rows for real overdue data — verified live end-to-end, including confirming a second call doesn't create duplicates (the functions' own idempotency guards hold). Setting `SUPABASE_SERVICE_ROLE_KEY`/`CRON_SECRET` on the live Vercel project (not just `.env.local`) is a manual step outside this session's write access — done locally and documented in `.env.example`; still needs to be added to the actual Vercel project for the cron to run in production.
...
### AE — Todo list (place/phase-taggable) — shipped (2026-09-21)
...
Built as: migrations `0032_todo_notification_type.sql` (the `todo_added` enum value, its own transaction per the `ALTER TYPE ... ADD VALUE` restriction) and `0033_todos.sql` (`todo_phase` enum, `todos` table, `todos_all_member` RLS policy, `notify_todo_added` trigger) — both applied live via `supabase db query --linked`. `src/lib/validation/todo.schema.ts` + `src/lib/queries/use-todos.ts` (`useTodos`/`useAddTodo`/`useUpdateTodo`/`useDeleteTodo`/`useToggleTodo`, no offline-snapshot fallback, matching `packing_items`) mirror the `tips`/`packing_items` hook shape. `src/components/todos/` holds `PhaseFilter.tsx` (a dedicated 3-value chip filter, not a reuse of `CategoryFilter` — phase needed label mapping a free-form string list doesn't), `TodoForm.tsx`, `TodoRow.tsx` (shared-only completion checkbox), and `TodosView.tsx` (self-contained, own add/edit/delete dialogs, embedded as the "Todo" tab's content). `PackingView.tsx` now wraps its existing item-matrix content and `TodosView` in `DetailTabs` under "Packing"/"Todo" tabs, with the page `<h1>` renamed to "Checklists"; `BottomNav.tsx`'s `packing` segment label changed to "Checklists" (URL unchanged). `PlaceDetail.tsx` gained a 4th "Todos" tab filtering by `related_place_id`, alongside Overview/Tips/Costs. `NotificationsView.tsx` and `PushNotificationSettings.tsx` got `todo_added` entries (both required by TypeScript for the former, a `Record<NotificationType, ...>`).

Verified live: `npx tsc --noEmit` clean, `npx eslint src` clean (one pre-existing unrelated warning), full Vitest suite 204/204 passing (including 8 new `todo.schema.test.ts` cases). Production-build Playwright run (two disposable accounts, a throwaway trip provisioned via privileged `supabase db query --linked` SQL since `trip_members` has had no client-side insert policy since migration `0006_reusable_invites.sql` closed that RLS gap) confirmed end-to-end: `BottomNav` shows "Checklists"; the Checklists page renders the Packing/Todo tabs; adding a todo with a phase and a place link persists and displays both tags correctly; the second account sees the new todo appear via realtime with no reload (818ms propagation) and receives the `todo_added` notification ("… added a todo: …"); toggling the shared completion checkbox persists; the linked place's own Detail page shows the todo under its new Todos tab. Throwaway trip and SQL scratch files cleaned up afterward; direct DB query confirmed zero leftover rows.
...
### AG — Map bottom sheet — shipped (2026-09-23)
...
Built as a new primitive, not a `Dialog.tsx` extension — `Dialog` is shaped for a centered modal from its CSS up (`place-items: center`, fixed max-width, no positioning prop, no transition system), and retrofitting bottom-anchor positioning onto it risked regressing every existing Dialog consumer (Add-a-tip, delete-confirm, etc.) for a use case they don't need. `src/components/ui/BottomSheet.tsx` lifts `Dialog`'s proven behavioral bones verbatim (tap-outside-to-close via backdrop `onClick`/`stopPropagation`, Escape-to-close, `role="dialog"`/`aria-modal`, and — non-negotiably — the `isolation: isolate; zIndex: 2000` backdrop fix for the known Chromium/Leaflet GPU-compositing bug `Dialog`'s own comment already documents) and adds new bottom-anchored CSS (`.sheet-backdrop`/`.sheet`/`.sheet-handle`, a `@keyframes sheet-slide-up`) built from existing Broadsheet tokens (`--color-surface`, `--radius-lg`, `--shadow-lg`, `--space-*`) since no sheet/drawer pattern existed in the design system before this. Swipe-down-to-close is new pointer-event drag-tracking on a dedicated `.sheet-handle` grip (translateY follows the drag live, snapping closed past an 80px threshold or back to rest otherwise).

`src/components/map/MarkerSheet.tsx` composes `BottomSheet` with the stop/place's town, the existing `CheckInControl`/`PlaceCheckInControl` (dropped in as-is — both are self-contained with their own hooks, and `MapView` already fetches the `checkins`/`placeCheckins` arrays they need), `OpenInGoogleMapsLink`, and a "View full details" link to the full Stop/Place Detail page. `MapView.tsx`'s two `<Marker><Popup>...</Popup></Marker>` blocks became bare `<Marker eventHandlers={{ click: ... }} />` setting a `selection` state, with `<MarkerSheet>` rendered once alongside the map. The small embedded maps (`StopAreaMap`, `LocationMapLoader`) were untouched — this is scoped to `MapView` only, per the milestone's own boundary.

Verified live: `npx tsc --noEmit` clean, `npx eslint src` clean (same one pre-existing unrelated warning; one real lint catch fixed along the way — an errant `setState` inside a `useEffect` in the first `BottomSheet` draft, resolved by resetting drag state directly in the pointer-up handler instead of an effect). Full Vitest suite 223/223 passing (no new unit-testable logic here — this milestone is interaction/UI, not calculation, matching CLAUDE.md's "don't over-test simple presentational components" guidance for the parts that are presentational, though the interaction itself needed live coverage). Production-build Playwright run (throwaway trip provisioned via privileged SQL, one disposable account, a stop and a place seeded near-identical coordinates so both markers stay in the tight-zoom viewport `fitBounds` produces for a single-stop trip) confirmed all 18 steps: tapping the stop marker opens the sheet with the correct name, the map (`.leaflet-container`) stays visible/interactive underneath, "Open in Google Maps" and "View full details" (correct `href`) both render; Escape closes it; tapping the place marker opens the sheet with the place's own content; clicking the backdrop (tap-outside) closes it; reopening and clicking "View full details" performs a real navigation to the Place Detail page, confirming the full-page path survives. (Swipe-down-to-close is implemented and reasoned through the same pointer-event model already proven for `PhotoUpload`/`AddStopForm`-style drag interactions elsewhere in the codebase, but wasn't asserted via synthetic headless pointer events — those proved unreliable to simulate deterministically for a CSS-transform-driven dismiss, unlike Escape/tap-outside, which are.) Throwaway trip and three orphaned trips from earlier failed debugging attempts (root-caused to test-data geometry — a lone stop collapses `fitBounds` to a zero-size box at max zoom, pushing a too-far-away test place off-screen, not an app bug) all cleaned up afterward; direct DB query confirmed zero leftover rows.
