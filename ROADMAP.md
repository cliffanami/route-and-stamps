...
**Deferred, explicit call:** stop order (waits on Milestone C existing) and promoting tip/budget categories from free-text to real per-trip managed lists (add/rename/archive) — the latter was the single biggest chunk of the original scope and isn't covered by this milestone's own acceptance criterion below, so it's parked as its own follow-up rather than ballooning this one.
