...
| **Maps** | Leaflet + `react-leaflet`, CARTO Voyager tile layer | Free, no API key, works with a service-worker cache strategy for offline tiles (PRD §4b) in a way a metered tile provider wouldn't let us do without a paid plan. Voyager rather than stock OSM tiles specifically: standard OSM raster tiles bake place labels into the image in the local script (e.g. 東京都 rather than Tokyo, for anywhere in Japan) with no way to request a different language, since tiles are pre-rendered PNGs cached by URL — Voyager renders the same OSM data with labels already romanized. A brief detour through the Google Maps JavaScript API was tried and reverted: every embedding path requires a Google Cloud project with billing enabled (not actually keyless despite the "free tier" framing) and prohibits caching tiles for offline use, conflicting with M6. Settled instead on Leaflet/CARTO for the embedded map plus a plain `google.com/maps` deep-link (`components/map/OpenInGoogleMapsLink.tsx`) for "open in native Google Maps app" — the deep-link is just a URL, not the JS API, so it stays free and keyless. |
...
**Tier 1 — free, always on, no toggle:** a local heuristic parser (`lib/geo/parse-place-mention.ts`, pure logic, no external call) strips narrative-filler prefixes ("posted a video about," "saw this on," "check this out," "you have to see," "reminds me of," bare hashtags/emoji, etc.), recognizes `<Name> in <City>` and reformats as `<Name>, <City>`, and otherwise passes text through unchanged — then hands the cleaned result straight to the **existing** Nominatim search Route Handler. No new endpoint; this is a preprocessing step in front of a call that already exists. Nominatim's own fuzzy matching does real work here too — a well-formed query like "KICC, Nairobi, Kenya" needs no extraction at all and resolves directly.

**Tier 2 — optional, toggled, cascading fallback only:** fires **only when Tier 1's Nominatim call returns zero results.** Settled during M1.5 build: "inconclusive" does *not* currently include multiple-ambiguous-matches — `PlaceForm` already takes the top result with no disambiguation UI today, and building real ambiguity scoring (comparing Nominatim `importance` values, exposing choices to the user) is a materially bigger UI change than this milestone's additive framing covers. Deferred, not forgotten — worth revisiting if silently-wrong top-result pins turn out to be a real problem in practice, at which point it's a UI-scoped follow-up, not a Tier 1/2 change. On an actual zero-result miss, a new Route Handler (`/api/extract-place`) calls the Claude API to separate name from location and expand abbreviations ("KICC" → "Kenyatta International Convention Centre"). Gated by `ENABLE_AI_PLACE_EXTRACTION` (env var, default **off**) — flip without a redeploy. Never the first call made; a fallback for the minority of inputs Tier 1 can't resolve at all, which keeps real cost low even with the toggle on.
...
**Cost backstop:** independent of the app-level toggle, set a hard per-key spend limit in the Anthropic Console before the toggle is ever flipped on for real — a real cutoff, not just an alert. Two independent safety nets, not one. (No `ANTHROPIC_API_KEY` needed until that point — Tier 2 stays untestable-but-harmless behind the default-off toggle in the meantime.)
